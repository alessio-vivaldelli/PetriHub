 ...
 *
 * <p>A computation tracks the execution of a Petri net from start to finish, maintaining
 * an ordered collection of computation steps that represent transitions fired during execution.
 * Each computation has a clear lifecycle: Created → Started → Finished.
 *
 * <p><strong>Lifecycle States:</strong>
 * <ul>
 * <li><strong>Created:</strong> Computation object exists but execution hasn't begun</li>
 * <li><strong>Started:</strong> Execution has begun, steps can be added</li>
 * <li><strong>Finished:</strong> Execution completed, no more steps can be added</li>
 * </ul>
 *
...
public class Computation {
     ...
     *
     * <p>The computation is not started and has no timestamps set. Use {@link #start()}
     * or {@link #start(long)} to begin execution.
     *
     * @param netId the unique identifier of the associated Petri net, must not be null or empty
     * @param creatorId the unique identifier of the user who created this computation, must not be null or empty
     * @param userId the unique identifier of the user executing this computation, must not be null or empty
    ...
    /**
     * Creates a computation with specified timestamps, typically used when loading from persistence.
     *
     * @param netId the unique identifier of the associated Petri net, must not be null or empty
     * @param creatorId the unique identifier of the user who created this computation, must not be null or empty
     * @param userId the unique identifier of the user executing this computation, must not be null or empty
     * @param startTimestamp the start time as Unix timestamp in seconds, or -1 if not started
     * @param endTimestamp the end time as Unix timestamp in seconds, or -1 if not finished
     * @throws IllegalArgumentException if any string parameter is null or empty, or if endTimestamp is before startTimestamp when both are positive
     */
     ...
     * @throws IllegalArgumentException if any string parameter is null or empty, or if endTimestamp is before startTimestamp when both are positive
     */
    public Computation(String netId, String creatorId, String userId, long startTimestamp) {
        this(netId, creatorId, userId);
     ...
     *
     * <p>Transitions the computation from Created to Started state. Once started,
     * computation steps can be added and the computation can eventually be finished.
     *
     ...
     */
    public void start() {
        if (isStarted())  throw new IllegalStateException("Computation is already started");
     ...
     *
     * @throws IllegalStateException if the computation is not started or is already finished
     * @see #finish(long)
     * @see #isFinished()
     * @see #getDurationSeconds()
     */
    public void finish() {
        if (!isStarted()) throw new IllegalStateException("Computation must be started before finishing");
        if (isFinished()) throw new IllegalStateException("Computation is already finished");
     ...
     * @throws IllegalStateException if the computation is not started or is already finished
     * @throws IllegalArgumentException if endTimestamp is before the start timestamp
     * @see #finish()
     * @see #isFinished()
     * @see #getDurationSeconds()
     */
    public void finish(long endTimestamp) {
        if (!isStarted()) throw new IllegalStateException("Computation must be started before finishing");
        if (isFinished()) throw new IllegalStateException("Computation is already finished");
        if (endTimestamp < startTimestamp) throw new IllegalArgumentException("End timestamp cannot be before start timestamp");
     ...
     * @return true if the step was successfully removed, false if the step was null or not found
     ...
     *
     * <p>This operation does not affect the computation's state (Created/Started/Finished),
     ...
     *
     * <p>Steps are automatically sorted by their timestamp in ascending order.
     * The returned set cannot be modified directly; use {@link #addStep(ComputationStep)}
     ...
     *
     * @return true if the computation is in Started or Finished state, false if in Created state
     * @see #start()
     * @see #isFinished()
     */
    ...
    public LocalDateTime getStartDate() {
        if (!isStarted()) throw new IllegalStateException("Computation has not started yet");
    ...
    public LocalDateTime getEndDate() {
        if (!isFinished()) throw new IllegalStateException("Computation is not finished yet");
